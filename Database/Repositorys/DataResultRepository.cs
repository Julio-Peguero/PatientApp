...
namespace Database.Repositorys
{
    public class DataResultRepository
    {
        ...
        {
            SqlDataAdapter query = new SqlDataAdapter();
            SqlCommand command = new SqlCommand("SELECT r.Id as Code,t.Name as Test,s.Status FROM ResultLab r JOIN Patients p ON r.IdPatients = p.Id JOIN TestLab t ON r.IdLabTest = t.Id JOIN ResultStatus s ON r.StateResult = s.Id WHERE r.IdKeep = @id", _connection);
